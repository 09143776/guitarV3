...
<%  
 ...
 if (sp.equals("add")){  
        ...
        String username=request.getParameter("r3");
      
       
        //将所选购商品加入到购物车中  
        sql = "insert into leoshop_cart(name,price,num,count,username) values(?,?,?,?,?)";  
    ...
    if (sp.equals("update")){  
        ...
        sql = "update leoshop_cart set num = ?, count = ? where id=?";  
        pstat = conn.prepareStatement(sql);  
        pstat.setInt(1,num);  
    ...
    if (sp.equals("del")){  
        int id = Integer.parseInt(request.getParameter("id"));  
        sql = "delete from leoshop_cart where id=?";  
        pstat = conn.prepareStatement(sql);  
        pstat.setInt(1,id);  
        pstat.executeUpdate();  
        pstat.close();    
